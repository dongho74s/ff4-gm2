...
class LongitudinalPlanner:
  ...
  def update(self, sm, carrot):
    ...
    self.a_desired = float(np.interp((self.dt + 0.017), CONTROL_N_T_IDX, self.a_desired_trajectory))
    self.v_desired_filter.x = self.v_desired_filter.x + (self.dt + 0.017) * (self.a_desired + a_prev) / 2.0
